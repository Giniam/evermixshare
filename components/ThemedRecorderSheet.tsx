...
export const ThemedRecorderSheet = forwardRef(
  (props: ThemedRecorderSheetProps, ref: Ref<TrueSheet>) => {
    const shouldUpload = process.env.UPLOAD_ON_STOP || true

    const { style, lightColor, darkColor, ...rest } = props

    const insets = useSafeAreaInsets()

    const [recordingId, setRecordingId] = useState('RECORDING_ID_NOT_SET')
    ...
    const uploadServer = `http://192.168.0.32:5075`
    ...
    const handleRecordStop = (record?: RecordInfo) => {
      ...
      console.log(`handleRecordStop: ${shouldUpload} uri=${record?.uri} duraction=${record?.duration}`)
      if (shouldUpload && record?.uri) {
        uploadToEvermix(uploadServer, recordingId, record?.uri)
      } else {
        console.log(`Cannot upload as shouldUpload=${shouldUpload}`, record?.uri)
      }
